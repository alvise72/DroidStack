...
public class CIDRAddressKeyListener extends NumberKeyListener {
 
    private char[] mAccepted;
    private static CIDRAddressKeyListener sInstance;
    ...
    private CIDRAddressKeyListener() {
     ...
     */
    synchronized public static CIDRAddressKeyListener getInstance() {
        if (sInstance != null) 
        	return sInstance;
        sInstance = new CIDRAddressKeyListener();
        return sInstance;
    }
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) 
    {
        ...
        // Prevent two slash characters in a row
        if (dstart > 0 && dest.charAt(dstart - 1) == '/') {
            decimal = dstart - 1;
        }
        if (dend < dlen && dest.charAt(dend) == '/') {
